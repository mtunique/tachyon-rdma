...
public class RemoteBlockInStream extends BlockInStream {
  ...
  private ByteBuffer retrieveByteBufferFromRemoteMachine(InetSocketAddress address, long blockId,
    ...
    String uri = String.format("rdma://%s:%d/blockId=%d", address.getHostName(), address.getPort(),blockId);
      try {
        JxioConnection connect = new JxioConnection(new URI(uri));
        connect.setRcvSize(655360);
